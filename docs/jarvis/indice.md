# Jarvis - Indice Tecnico

- [`visao.md`](visao.md)
- [`origem-e-upstream.md`](origem-e-upstream.md)
- [`arquitetura/arquitetura-em-camadas.md`](arquitetura/arquitetura-em-camadas.md)
- [`agentes/mapeamento-e-orquestracao.md`](agentes/mapeamento-e-orquestracao.md)
- [`ferramentas/tools-skills-mcp.md`](ferramentas/tools-skills-mcp.md)
- [`modelos/model-router.md`](modelos/model-router.md)
- [`seguranca/governanca-e-auditoria.md`](seguranca/governanca-e-auditoria.md)
- [`integracao/contratos-e-consolidacao.md`](integracao/contratos-e-consolidacao.md)
- [`desenvolvimento/roadmap-e-gates.md`](desenvolvimento/roadmap-e-gates.md)

## Estado

Esta pasta descreve a arquitetura desejada do Jarvis standalone. O repositorio derivado do OpenJarvis ainda precisa ser criado/importado e auditado antes que qualquer capacidade seja marcada como implementada.

